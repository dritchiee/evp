...
public class IPRangeSetTest
{
	private void assertSubnets(IPRangeSet set, IPRange...exp)
	{
		...
		if (exp.length == 0)
		{
			Assert.assertEquals("no subnets", false, subnets.hasNext());
			return;
		}
		for (IPRange e : exp)
		{
			Assert.assertEquals("has subnet", true, subnets.hasNext());
			assertEquals("range", e, subnets.next());
		}
		Assert.assertEquals("done", false, subnets.hasNext());
	}
	...
	public void testIteratorRanges() throws UnknownHostException
	{
		...
		Assert.assertEquals("hasNext", true, iterator.hasNext());
		assertEquals("next", new IPRange("192.168.1.0-192.168.2.255"), iterator.next());
		Assert.assertEquals("hasNext", false, iterator.hasNext());
	}
