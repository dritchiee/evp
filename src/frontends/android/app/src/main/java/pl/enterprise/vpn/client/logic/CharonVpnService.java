...
import pl.enterprise.vpn.client.logic.imc.ImcState;
import pl.enterprise.vpn.client.logic.imc.RemediationInstruction;
import pl.enterprise.vpn.client.ui.MainActivity;
import pl.enterprise.vpn.client.utils.IPRange;
import pl.enterprise.vpn.client.utils.IPRangeSet;
import pl.enterprise.vpn.client.utils.SettingsWriter;
...
import static pl.enterprise.vpn.client.logic.ManagedConfigurationContract.Controller.HAS_MANAGED_CONFIG;
...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener
{
			...
			synchronized (this)
			{
				try
				{
					...
					else
					{
						...
						if (initializeCharon(builder, mLogFile, mAppDir, mCurrentProfile.getVpnType().has(VpnType.VpnTypeFeature.BYOD)))
						{
	...
	private Notification buildNotification(boolean publicVersion)
	{
		...
		android.support.v4.app.NotificationCompat.Builder builder = new NotificationCompat.Builder(this)
				.setSmallIcon(pl.enterprise.vpn.client.R.drawable.ic_notification)
				.setCategory(NotificationCompat.CATEGORY_SERVICE)
				.setVisibility(publicVersion ? NotificationCompat.VISIBILITY_PUBLIC
											 : NotificationCompat.VISIBILITY_PRIVATE);
		int s = pl.enterprise.vpn.client.R.string.state_disabled;
		if (error != VpnStateService.ErrorState.NO_ERROR)
		{
			s = pl.enterprise.vpn.client.R.string.state_error;
			builder.setSmallIcon(pl.enterprise.vpn.client.R.drawable.ic_notification_warning);
			builder.setColor(ContextCompat.getColor(this, pl.enterprise.vpn.client.R.color.error_text));
		}
		else
		{
			switch (state)
			{
				case CONNECTING:
					s = pl.enterprise.vpn.client.R.string.state_connecting;
					builder.setSmallIcon(pl.enterprise.vpn.client.R.drawable.ic_notification_warning);
					builder.setColor(ContextCompat.getColor(this, pl.enterprise.vpn.client.R.color.warning_text));
					add_action = true;
					break;
				case CONNECTED:
					s = pl.enterprise.vpn.client.R.string.state_connected;
					builder.setColor(ContextCompat.getColor(this, pl.enterprise.vpn.client.R.color.success_text));
					builder.setUsesChronometer(true);
					add_action = true;
					break;
				case DISCONNECTING:
					s = pl.enterprise.vpn.client.R.string.state_disconnecting;
					break;
			}
		}
		...
		if (!publicVersion)
		{
			if (add_action)
			{
				...
				builder.addAction(pl.enterprise.vpn.client.R.drawable.ic_notification_disconnect, getString(pl.enterprise.vpn.client.R.string.disconnect), pending);
			}
