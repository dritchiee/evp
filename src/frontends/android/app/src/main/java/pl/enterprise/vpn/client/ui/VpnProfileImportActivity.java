...
import pl.enterprise.vpn.client.logic.TrustedCertificateManager;
import pl.enterprise.vpn.client.security.TrustedCertificateEntry;
import pl.enterprise.vpn.client.ui.widget.TextInputLayoutHelper;
import pl.enterprise.vpn.client.utils.Constants;
import pl.enterprise.vpn.client.utils.IPRangeSet;
...
public class VpnProfileImportActivity extends AppCompatActivity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		getSupportActionBar().setHomeAsUpIndicator(pl.enterprise.vpn.client.R.drawable.ic_close_white_24dp);
		getSupportActionBar().setDisplayHomeAsUpEnabled(true);

		mDataSource = new VpnProfileDataSource(this);
		mDataSource.open();

		setContentView(pl.enterprise.vpn.client.R.layout.profile_import_view);

		mExistsWarning = (TextView)findViewById(pl.enterprise.vpn.client.R.id.exists_warning);
		mBasicDataGroup = (ViewGroup)findViewById(pl.enterprise.vpn.client.R.id.basic_data_group);
		mName = (TextView)findViewById(pl.enterprise.vpn.client.R.id.name);
		mGateway = (TextView)findViewById(pl.enterprise.vpn.client.R.id.gateway);
		mSelectVpnType = (TextView)findViewById(pl.enterprise.vpn.client.R.id.vpn_type);

		mUsernamePassword = (ViewGroup)findViewById(pl.enterprise.vpn.client.R.id.username_password_group);
		mUsername = (EditText)findViewById(pl.enterprise.vpn.client.R.id.username);
		mUsernameWrap = (TextInputLayoutHelper) findViewById(pl.enterprise.vpn.client.R.id.username_wrap);
		mPassword = (EditText)findViewById(pl.enterprise.vpn.client.R.id.password);

		mUserCertificate = (ViewGroup)findViewById(pl.enterprise.vpn.client.R.id.user_certificate_group);
		mSelectUserCert = (RelativeLayout)findViewById(pl.enterprise.vpn.client.R.id.select_user_certificate);
		mImportUserCert = (Button)findViewById(pl.enterprise.vpn.client.R.id.import_user_certificate);

		mRemoteCertificate = (ViewGroup)findViewById(pl.enterprise.vpn.client.R.id.remote_certificate_group);
		mRemoteCert = (RelativeLayout)findViewById(pl.enterprise.vpn.client.R.id.remote_certificate);
		...
		mImportUserCert.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v)
			{
				...
				intent.putExtra(KeyChain.EXTRA_NAME, getString(pl.enterprise.vpn.client.R.string.profile_cert_alias, mProfile.getName()));
	...
	public boolean onCreateOptionsMenu(Menu menu)
	{
		...
		inflater.inflate(pl.enterprise.vpn.client.R.menu.profile_import, menu);
		if (mHideImport)
		{
			MenuItem item = menu.findItem(pl.enterprise.vpn.client.R.id.menu_accept);
	...
	public boolean onOptionsItemSelected(MenuItem item)
	{
		switch (item.getItemId())
		{
			case android.R.id.home:
				finish();
				return true;
			case pl.enterprise.vpn.client.R.id.menu_accept:
				saveProfile();
				return true;
			default:
	...
	{
		mProgress = ProgressDialog.show(this, null, getString(pl.enterprise.vpn.client.R.string.loading),
	...
	public void handleProfile(ProfileLoadResult data)
	{
		...
		if (mProfile == null)
		{
			...
			if (data.ThrownException != null)
			{
				...
				{
					error = getString(pl.enterprise.vpn.client.R.string.profile_import_failed_not_found);
				}
				catch (UnknownHostException e)
				{
					error = getString(pl.enterprise.vpn.client.R.string.profile_import_failed_host);
				}
				catch (SSLHandshakeException e)
				{
					error = getString(pl.enterprise.vpn.client.R.string.profile_import_failed_tls);
				}
				catch (Exception e)
				{
					e.printStackTrace();
			...
			if (error != null)
			{
				Toast.makeText(this, getString(pl.enterprise.vpn.client.R.string.profile_import_failed_detail, error), Toast.LENGTH_LONG).show();
			}
			else
			{
				Toast.makeText(this, pl.enterprise.vpn.client.R.string.profile_import_failed, Toast.LENGTH_LONG).show();
			}
			finish();
			return;
		}
		...
		mSelectVpnType.setText(getResources().getStringArray(pl.enterprise.vpn.client.R.array.vpn_types)[mProfile.getVpnType().ordinal()]);

		mUsernamePassword.setVisibility(mProfile.getVpnType().has(VpnType.VpnTypeFeature.USER_PASS) ? View.VISIBLE : View.GONE);
		...
		mUserCertificate.setVisibility(mProfile.getVpnType().has(VpnType.VpnTypeFeature.CERTIFICATE) ? View.VISIBLE : View.GONE);
		...
		if (mProfile.getVpnType().has(VpnType.VpnTypeFeature.CERTIFICATE))
		{	/* try to load an existing certificate with the default name */
			if (mUserCertLoading == null)
			{
				mUserCertLoading = getString(pl.enterprise.vpn.client.R.string.profile_cert_alias, mProfile.getName());
				getLoaderManager().initLoader(USER_CERT_LOADER, null, mUserCertificateLoaderCallbacks);
			}
			updateUserCertView();
		}
	...
	private void updateUserCertView()
	{
		if (mUserCertLoading != null)
		{
			...
			((TextView)mSelectUserCert.findViewById(android.R.id.text2)).setText(pl.enterprise.vpn.client.R.string.loading);
		}
		...
		{
			((TextView)mSelectUserCert.findViewById(android.R.id.text1)).setText(pl.enterprise.vpn.client.R.string.profile_user_select_certificate_label);
			((TextView)mSelectUserCert.findViewById(android.R.id.text2)).setText(pl.enterprise.vpn.client.R.string.profile_user_select_certificate);
		}
	}
	...
	private ParsedVpnProfile parseProfile(JSONObject obj) throws JSONException
	{
		...
		if (local != null)
		{
			if (type.has(VpnType.VpnTypeFeature.USER_PASS))
			{
				profile.setUsername(local.optString("eap_id", null));
			}

			if (type.has(VpnType.VpnTypeFeature.CERTIFICATE))
			{
	...
	private String getSubnets(JSONObject split, String key) throws JSONException
	{
		...
		if (subnets.size() > 0)
		{
			...
			if (ranges == null)
			{
				throw new JSONException(getString(pl.enterprise.vpn.client.R.string.profile_import_failed_value,
												  "split-tunneling." + key));
			}
	...
	private boolean verifyInput()
	{
		...
		if (mProfile.getVpnType().has(VpnType.VpnTypeFeature.USER_PASS))
		{
			...
			{
				mUsernameWrap.setError(getString(pl.enterprise.vpn.client.R.string.alert_text_no_input_username));
				valid = false;
			}
		}
		if (mProfile.getVpnType().has(VpnType.VpnTypeFeature.CERTIFICATE) && mUserCertEntry == null)
	...
	private class SelectUserCertOnClickListener implements View.OnClickListener, KeyChainAliasCallback
	{
		...
		public void onClick(View v)
		{
			...
			{
				alias = getString(pl.enterprise.vpn.client.R.string.profile_cert_alias, mProfile.getName());
			}
